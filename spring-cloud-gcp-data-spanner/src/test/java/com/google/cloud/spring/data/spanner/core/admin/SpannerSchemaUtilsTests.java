...
class SpannerSchemaUtilsTests {
  ...
  void getCreateDdlTest() {
    ...
    verifyCreateDdl(ddlResult, this.spannerSchemaUtils.getCreateTableDdlString(TestEntity.class));
  }
   ...
   *
   * @param actualDdl The actual DDL string to verify.
   * @param expectedDdl The expected DDL string.
   */
  private static void verifyCreateDdl(String actualDdl, String expectedDdl) {
    CreateDdlParts actualDdlParts = parseCreateDdl(actualDdl);
    CreateDdlParts expectedDdlParts = parseCreateDdl(expectedDdl);
  ...
  /**
   * Parses a create DDL statement into pre-column, column definition, and post-column parts.
   *
   * @param createDdl The create DDL statement to parse.
   */
  private static CreateDdlParts parseCreateDdl(String createDdl) {
    createDdl = createDdl.replaceAll("\\s+", " ").trim();

    String preCols = createDdl.substring(
            createDdl.indexOf("CREATE TABLE"),
            createDdl.indexOf("(")).trim();

    String colsString = createDdl.substring(
            createDdl.indexOf("(") + 1,
            createDdl.lastIndexOf(") PRIMARY KEY")).trim();

    String postCols = createDdl.substring(
            createDdl.indexOf("PRIMARY KEY (")).trim();
    ...
    return new CreateDdlParts(preCols, columns, postCols);
  }

  private static class CreateDdlParts {
    final String preCols;
    final Set<String> columns;
    final String postCols;

    CreateDdlParts(String preCols, Set<String> columns, String postCols) {
